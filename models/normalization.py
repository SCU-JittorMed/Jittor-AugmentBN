...
#         self.gamma = nn.Parameter(jt.ones(shape))
#         self.beta = nn.Parameter(jt.zeros(shape))
#         self.moving_mean = jt.zeros(shape)
#         self.moving_var = jt.ones(shape)
#         self.momentum = momentum
#         self.eps = eps
#         self.theta = theta
#         print('self.theta is', self.theta)
...
#         # if not jt.grad_enabled():
#         # if jt.has_grad() == False:
#         if self.is_train:

#             X_hat = (X - self.moving_mean) / jt.sqrt(self.moving_var + self.eps)
#         else:
...
#                 noise = jt.randn(mean.shape)
#                 mean_normal = jt.maximum(mean - self.theta, jt.minimum(mean + self.theta, mean + noise))
#                 mean = mean_normal
#                 var = ((X - mean)**2).mean(dim=(0, 2, 3), keepdims=True)

#             X_hat = (X - mean) / jt.sqrt(var + self.eps)
...
class GroupNorm2d(nn.Module):
    ...
    def execute(self, X):
        ...
        var = ((X - mean)**2).mean(dim=2, keepdims=True)
        X_hat = (X - mean) / jt.sqrt(var + self.eps)
    ...
    def execute(self, X):
        ...
        if not jt.grad_enabled():
            ...
            if len(X.shape) == 2:
                ...
                mean = X.mean(dim=(0, 2, 3), keepdims=True)
                mean_uniform = self.theta * 2 * jt.rand(mean.shape) + (mean - self.theta)
                mean = mean_uniform
                var = ((X - mean)**2).mean(dim=(0, 2, 3), keepdims=True)
